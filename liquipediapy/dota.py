...
def get_links(links_html):
    result = {}
    links = links_html.find_all("a")
    for link in links:
        ...
        result[site_name] = link.get("href")
    return result


def parse_player(country, player_html):
    id_html, name_html, team_html, links_html = player_html

    return {
        "country": country,
        "ID": id_html.find("a").get("title"),
        "Name": name_html.get_text().strip(),
        "Team": team_html.get_text().strip(),
        "Links":  get_links(links_html),
    }
...
class dota:
    ...
    def get_players(self):
        ...
        players = []
        country = rows[0].find("a").get("title")
        for row in rows[1:]:
            cells = row.find_all("td")
            if not cells and not row.find("a"):
                # is column headers
                continue
            elif not cells:
                # is country html
    ...
    def get_upcoming_and_ongoing_games(self):
        ...
        for match in matches:
            ...
            try:
                game["team1"] = (
                    cells[0]
                    .find("span", class_="team-template-image-icon")
                    .find("a")
                    .get("title")
                )
    ...
    def get_pro_circuit_details(self):
        soup, __ = self.liquipedia.parse(
            "Dota_Pro_Circuit/2018-19/Rankings/Full")
        pro_circuit = {}
